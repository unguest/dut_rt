...
long long int recursiveSquare(long long int nombre);
int loopPower(int nombre, int puissance);
long long int recursivePower(long long int nombre, int puissance);
long long int optimizedRecursivePower(long long int nombre, int puissance);
...
int main(void) {
    ...
    t = clock();
    long long int resultRPower = recursivePower(nombre, puissance);
    t = clock() - t;
    printf("%d ^ %d = %lld\n", nombre, puissance, resultRPower);
    long double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
    printf("recursivePower() took %Lf seconds to execute \n", time_taken);

    // OPTIMIZED RECURSIVE FUNCTION

    t = clock();
    long long int optimizedPower = optimizedRecursivePower(nombre, puissance);
    t = clock() - t;

    printf("%d ^ %d = %lld\n", nombre, puissance, optimizedPower);

    time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
    printf("optimizedRecursivePower() took %Lf seconds to execute \n", time_taken);
